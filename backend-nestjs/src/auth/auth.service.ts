...
export class AuthService {
    ...
    async verifyEmailAndAuthorize(code: number) { // Функция для верификации пользователя по email и коду
        ...
        await this.prisma.tokensTable.create({ data: { user_id: userInDb.id, refreshToken: tokens.refreshToken } })
    ...
    async verifyUserAuth(code: number, req: Request) {
        ...
        await this.prisma.tokensTable.create({ data: { user_id: userDto.id, refreshToken: tokens.refreshToken } })
